...
const {B,S,isObj,notNil} = core.types;

const webcore = require('@lumjs/web-core');
const parser = webcore.parser;

const {STANDARD_HTTP,PLACEHOLDERS,MIME,OBSERV_DEFS} = require('./defs');
...
class WebserviceMethodCall
{
   ...
   * 
   * In addition to any options valid for the `Webservice`
   * class, this also supports the `observable`, `eventRegistry`,
   * and `observableRedefine` options from the `Builder` class.
   * 
   */
   ...
   * 
   * Will automatically call `this.makeObservable()`,
   * after setting the `ws` property.
   * 
   * @protected
   ...
   * 
   * Generally never needs to be called manually, as `setWS()` will
  ...
  makeObservable()
  {
    let ro = this.getOption('observableRedefine');
    if (typeof ro !== B)
    {
      ro = this.getNestedOptions('eventRegistry', {defaults: ER_DEFS});
    }

    const obsDefs = {defaults: OBSERV_DEFS}
    const obsOpts = this.getNestedOptions('observable', obsDefs);

    core.observable(this, obsOpts, ro);
    return this;
  }
