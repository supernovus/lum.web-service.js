...
const AUTO_IDS = 
{
  __globalInstance: null,
  getInstance()
  {
    if (!this.__globalInstance)
    {
      this.__globalInstance = new core.UniqueObjectIds(
      {
        idProperty: '_id',
        className(name)
        {
    ...
    return this.__globalInstance;
  },
}

/**
 * Default options for applying `observable()`;
 * used by `MethodCall` and `Builder` instances.
 * 
 * - `wrapargs: true`
 * - `wrapthis: false`
 * - `wraplock: false`
 * 
 * @alias module@lumjs/web-service/defs.OBSERV_DEFS
 */
const OBSERV_DEFS =
{
  wrapargs: true,
...
module.exports =
{
  ...
  AUTO_IDS, AUTO_ID_STRIP, OBSERV_DEFS,
}
